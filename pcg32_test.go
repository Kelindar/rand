...
func TestSanity32(t *testing.T) {
	result := newRand32().Seed(1, 1).Random()
	expect := uint32(3380776849)
	if result != expect {
		t.Errorf("newRand32().Seed(1, 1).Random() is %d; want %d", result, expect)
	}
}
...
func TestSum32(t *testing.T) {
	for i, a := range sumTests32 {
		pcg := newRand32().Seed(a.state, a.sequence)
		sum := uint64(0)
		for j := 0; j < a.count; j++ {
