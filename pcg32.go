...
func newRand32() *rand32 {
	return &rand32{pcg32State, pcg32Increment}
}

func (p *rand32) Seed(state, sequence uint64) *rand32 {
	p.increment = (sequence << 1) | 1
	p.state = (state+p.increment)*pcg32Multiplier + p.increment
	return p
}

func (p *rand32) Random() uint32 {
...
func (p *rand32) Bounded(bound uint32) uint32 {
	if bound == 0 {
		return 0
	}
...
func (p *rand32) Advance(delta uint64) *rand32 {
	p.state = p.advanceLCG64(p.state, delta, pcg32Multiplier, p.increment)
	return p
}

func (p *rand32) Retreat(delta uint64) *rand32 {
	return p.Advance(-delta)
}

func (p *rand32) advanceLCG64(state, delta, curMult, curPlus uint64) uint64 {
	accMult := uint64(1)
	accPlus := uint64(0)
	for delta > 0 {
		if delta&1 != 0 {
