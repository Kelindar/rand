...
func TestSanity64(t *testing.T) {
	result := newRand64().Seed(1, 1, 1, 2).Random()
	expect := uint64(1107300197865787281)
	if result != expect {
		t.Errorf("newRand64().Seed(1, 1, 1, 2).Random() is %q; want %q", result, expect)
	}
}
...
func TestSum64(t *testing.T) {
	for i, a := range sumTests64 {
		pcg := newRand64().Seed(a.state, a.state, a.sequence, a.sequence+1)
		sum := uint64(0)
		for j := 0; j < a.count; j++ {
...
func ExampleReport64() {
	// Print report
	rng := newRand64().Seed(42, 42, 54, 54)
